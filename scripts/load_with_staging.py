...
class StagingTableLoader:
    ...
    NUMERIC_COLUMNS = {
        "events": [
            ...
            "ev_nr_apt_dist", "dec_latitude", "dec_longitude",
        ],
    ...
    def __init__(self, source_db: str, mdb_path: str, force_reload: bool = False):
        ...
        self.csv_dir = Path('data') / source_db.replace('.', '_').replace(' ', '_').lower()
        self.conn = None
        self.cursor = None
    ...
    def check_load_status(self) -> str:
        ...
        if result:
            ...
            if status == 'completed' and completed_at:
                ...
                logger.error("✗ Historical databases (PRE1982, Pre2008) should only load ONCE")
    ...
    def extract_csv(self, table_name: str) -> Optional[Path]:
        ...
        try:
            ...
            if row_count == 0:
                logger.warning(f"  ⊘ Empty table, skipping")
                return None
            ...
            logger.error(f"  ✗ Extraction timeout (>5 minutes)")
            return None
    ...
    def clean_dataframe(self, df: pd.DataFrame, table_name: str) -> pd.DataFrame:
        ...
        # Clean common issues
        df = df.replace({'': None, 'UNK': None, 'UNKN': None, 'NONE': None, 'nan': None})
        ...
        if table_name == 'Findings' and 'cause_factor' in df.columns:
            ...
            df['cm_inpc'] = df['cause_factor'].apply(
                lambda x: True if (pd.notna(x) and str(x).strip().upper() == 'C') else (
        ...
        if table_name == 'events':
            ...
            if 'crew_age' in df.columns:
                ...
                if invalid_count > 0:
                    logger.warning(f"  ⚠ Converting {invalid_count} invalid crew ages to NULL")
            ...
            if 'occurrence_code' in df.columns:
                df['occurrence_code'] = pd.to_numeric(df['occurrence_code'], errors='coerce')
                df.loc[(df['occurrence_code'] <= 0) | (df['occurrence_code'] > 999), 'occurrence_code'] = None
        ...
        if cleaned_count < original_count:
            logger.warning(f"  ⚠ Removed {original_count - cleaned_count} invalid rows during cleaning")
...
"""
        ...
        # Save to file
        report_file = Path('docs') / f"{self.source_db.replace('.', '_')}_load_report.txt"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Load NTSB historical data with staging tables')
